...
- **2026-09-12 — Bill Now built, used, withdrawn the same day; production
  restored to the 11:43 backup.** See CHANGELOG. Open gaps it exposed, now
  on the list below: no reversal path for a mis-recorded payment; Restore's
  two-stage apply-at-restart needs a dialog, not just a banner.
...
smoke-tested clean. Tag `v2.0.0` at `5100af3`. Website overhauled against the
code (every page read; schema/routes now generated by
`tools/gen_site_reference.py`), reviewed cold by Opus 5, and **pushed live
2026-08-22 evening** (`2d386a6`). Website clones on Apollo and
`~/Applications/websites/` need a `git pull`.
