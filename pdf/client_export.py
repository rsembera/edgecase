...
def generate_single_entry_pdf(db, client_id, entry_id):
    """
    Generate a PDF for a single session entry.
    ...
    Raises:
        ValueError: If entry not found or not a session
    """
    # Get client info
    client = db.get_client(client_id)
    if not client:
    ...
    # Only support session entries
    if entry_class != 'session':
        raise ValueError(f"PDF view only supported for session entries")
    ...
    styles = get_styles()
    elements = build_session_entry(entry, client, styles, signature_path, db)
