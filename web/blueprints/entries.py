...
# ============================================================================
# SINGLE ENTRY PDF VIEW
# ============================================================================

@entries_bp.route('/client/<int:client_id>/entry/<int:entry_id>/pdf')
def view_entry_pdf(client_id, entry_id):
    """Generate and serve a PDF for a single entry."""
    from pdf.client_export import generate_single_entry_pdf
    
    client = db.get_client(client_id)
    if not client:
        return "Client not found", 404
    
    # Get the entry to validate and get info for filename
    entry = db.get_entry(entry_id)
    if not entry or entry['client_id'] != client_id:
        return "Entry not found", 404
    ...
    if not entry.get('locked'):
        return "PDF view only available for locked entries", 400
    
    try:
        pdf_buffer = generate_single_entry_pdf(db, client_id, entry_id)
        
        # Create filename based on entry type and description
        file_number = client['file_number'].replace(' ', '_').replace('/', '-')
        description = entry.get('description', entry_class).replace(' ', '_').replace('/', '-')
        filename = f"{file_number}_{description}.pdf"
