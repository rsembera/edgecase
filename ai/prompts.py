...
- Preserve the writer's punctuation and style conventions. Regional style differences are not errors, in either direction. Critical examples: if the notes say "e.g. paced breathing", output exactly "e.g. paced breathing"; if the notes say "e.g., paced breathing", output exactly "e.g., paced breathing". Both are correct styles - never add the comma and never remove it. The same applies to "i.e." and to serial commas. Malformed abbreviations are still errors and must be fixed: "e.g" becomes "e.g." and "i.e" becomes "i.e." (keeping the writer's comma choice unchanged). Only correct punctuation that is wrong in every English convention.
